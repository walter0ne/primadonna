...
const props = defineProps({
  category: { type: String, default: null }, // 'corti_medi' | 'lunghi' | null = tutti
})
...
const filtered = computed(() =>
  props.category
    ? services.value.filter(s => (s.category || 'corti_medi') === props.category)
    : services.value
)
</script>
...
<template>
  <div>
    ...
    <div v-else-if="filtered.length === 0" class="text-center py-12 text-primary/40 text-sm">
